...
module Y
  class Redis
    class Config
      # Y::Redis configuration option DSL
      module Option
        ...
        def option(name, options = {}) # rubocop:disable Metrics/AbcSize, Metrics/CyclomaticComplexity, Metrics/MethodLength, Metrics/PerceivedComplexity
          ...
          builder_class.instance_eval do
            if method_defined?(name)
              Kernel.warn "[Y::Redis] Option #{name} already defined and will be overridden"
              remove_method name
            end

            define_method name do |*args, &block| # rubocop:disable Metrics/MethodLength
              if (deprecation_opts = options[:deprecated])
                warning = "[Y::Redis] #{name} has been deprecated and will soon be removed"
        ...
        def self.extended(base)
          ...
          raise Y::Redis::MissingConfigurationBuilderClass,
                "Define `self.builder_class` method " \
                "for #{base} that returns your custom Builder class to use " \
                "options DSL!"
        end
      end
    end
